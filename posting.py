...
def update_post(id):
    article = Article.query.get_or_404(id)
    if 'id' not in session or session['id'] != article.user_id:
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        try:
            ...
            flash("Error while updating the post.", "danger")
            return redirect(url_for('update_post', id=id))
    return render_template('post_update.html', form=form, post=article)